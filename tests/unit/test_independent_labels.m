...
function testManualEditVersionOneMigrationUsesFieldIdentity(testCase)
    ...
    verifyTrue(testCase, any(strcmp({info.review_history(1).events.type}, 'rapid')));
end
...
function [data, phys_feat, config] = detector_fixture()
    ...
    lungs = struct( ...
        ...
        'apnea_amplitude_evaluable_endpoint_mask', state, ...
        'apnea_amplitude_endpoint_mask', state, ...
        'apnea_amplitude_state_mask', state);
    diaph = lungs;
    diaph_peak_t = (0:6:138)';
    diaph.peak_t = diaph_peak_t;
    diaph.trough_t = 0.5 * (diaph_peak_t(1:end-1) + diaph_peak_t(2:end));
