...
function testPeriodicDetectorComputesBothAndSelectsExplicitPrimary(testCase)
    ...
    [events_eami, eami_primary] = detect_periodic_breathing( ...
        data, resp_cycles, config);
    config.csr.primary_method = 'guyot';
    [events_guyot, guyot_primary] = detect_periodic_breathing( ...
        data, resp_cycles, config);

    verifyTrue(testCase, isfield(eami_primary, 'eami'));
    verifyTrue(testCase, isfield(eami_primary, 'guyot'));
    verifyEqual(testCase, events_eami, eami_primary.eami.combined.events);
    verifyEqual(testCase, events_guyot, guyot_primary.guyot.combined.events);
...
function testEvidenceConsumersUseBothLiteratureMethods(testCase)
    ...
    signal_source = fileread(fullfile(repo_root, 'src', 'utils', ...
        'compute_label_diagnostic_signals.m'));
    required_summary = {'eami_available', 'eami_candidate_event_count', ...
        'eami_candidate_duration_sec', 'eami_max', 'eami_median', ...
        'guyot_available', 'guyot_candidate_event_count', ...
        'guyot_candidate_duration_sec', 'guyot_max_h', ...
        'guyot_median_h', 'guyot_median_fm_mhz'};
    required_signals = {'periodic_primary_method', 'periodic_eami_lungs', ...
        'periodic_eami_diaph', 'periodic_guyot_h_lungs', ...
        'periodic_guyot_h_diaph', 'periodic_guyot_fm_mhz_lungs', ...
        'periodic_guyot_fm_mhz_diaph'};
    ...
    for i = 1:numel(required_signals)
        verifyTrue(testCase, contains(signal_source, required_signals{i}));
    end
end
