...
function testAsynchronyDiagnosticPlotUsesOptionalBottomLegacyPanel(testCase)
    config = get_config_defaults();
    verifyFalse(testCase, config.async.plot_legacy_coherence);
    verifyEqual(testCase, respiratory_asynchrony_plot_panels(config), ...
        {'belts', 'phase_offset', 'phase_consistency'});

    config.async.plot_legacy_coherence = true;
    panels = respiratory_asynchrony_plot_panels(config);
    verifyNumElements(testCase, panels, 4);
    verifyEqual(testCase, panels{end}, 'legacy_coherence');

    repo_root = fileparts(fileparts(fileparts(mfilename('fullpath'))));
    source = fileread(fullfile(repo_root, 'src', 'label_detection', ...
        'detect_respiratory_asynchrony.m'));

    required = { ...
        'respiratory_asynchrony_plot_panels(config)', ...
        'tiledlayout(numel(panels), 1', ...
        'for i = 1:numel(panels)', ...
        'case ''legacy_coherence''', ...
        'Respiratory effort belts + final asynchrony events', ...
        'Normalized effort', ...
        '''DisplayName'', ''lungs''', ...
        '''DisplayName'', ''diaphragm''', ...
        '''DisplayName'', ''final async event''', ...
        'legacy_full.phase_coherence_mid', ...
        'legacy_full.thresholds.mid', ...
        'legacy_full.references.mid', ...
        'legacy events may also be triggered by', ...
        'Wavelet phase offset at shared respiratory frequency', ...
        'Phase consistency and shared respiratory frequency', ...
        '1.4826 * median(abs(centered))', ...
        'std(finite_signal, 0)'};
    ...
    removed_titles = { ...
        'Resp-Lungs with canonical async events', ...
        'Resp-Diaphragm with canonical async events', ...
        'Legacy high-frequency coherence', ...
        'Legacy low-frequency coherence'};
    for i = 1:numel(removed_titles)
        verifyFalse(testCase, contains(source, removed_titles{i}));
    end
end

function testLegacyPlotOptionDoesNotChangeDetectorOutputs(testCase)
    [data, config] = phase_fixture(20, 1);
    reference = struct('available', false);
    config.async.plot_legacy_coherence = false;
    [events_hidden, diagnostics_hidden] = detect_respiratory_asynchrony( ...
        data, reference, [], config);

    config.async.plot_legacy_coherence = true;
    [events_shown, diagnostics_shown] = detect_respiratory_asynchrony( ...
        data, reference, [], config);

    verifyEqual(testCase, events_shown, events_hidden);
    verifyEqual(testCase, diagnostics_shown, diagnostics_hidden);
end
...
function testFixedPolarityMultipliersAreAppliedWithoutOptimization(testCase)
    ...
    verifyEqual(testCase, recorded.polarity_source, ...
        'preprocessing_standardized_belt_polarity');
    verifyEqual(testCase, median( ...
    ...
    verifyEqual(testCase, corrected.polarity_source, ...
        ['preprocessing_standardized_then_' ...
