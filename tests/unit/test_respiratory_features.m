...
function testSighDiagnosticPlotShowsRawAndRatioEvidencePerBelt(testCase)
    ...
    required = { ...
        ...
        'Sigh amplitude-ratio evidence (%s)', ...
        'belt_diagnostics.amp_ratio_global', ...
        'belt_diagnostics.decision_threshold', ...
        '''LineStyle'', ''none''', ...
        '''DisplayName'', ''breath ratios''', ...
        '''DisplayName'', ''decision threshold''', ...
        '''DisplayName'', ''sigh breaths''', ...
        'No usable breath-ratio evidence', ...
        'Breath amplitude / global reference'};
    for i = 1:numel(required)
        verifyTrue(testCase, contains(source, required{i}));
    end
    verifyEqual(testCase, count(source, 'subplot(4,1'), 4);
