...
Breath amplitude, raw respiratory-belt excursion, SpO2, and respiratory-asynchrony coherence each calculate their own statistic from this same interval. Sharing the time interval does not share a numerical normalization scale across modalities, and an unavailable statistic for one modality does not invalidate the others. The complementary asynchrony phase-offset method is intentionally not normalized to this interval, and the absolute SpO2 floor remains evaluable when the interval-specific SpO2 reference is unavailable. Reference-quality warnings retain the data by default and do not trigger automatic correction, interval movement, or threshold tuning.
...
- **`sigh`** — isolated unusually large breaths are detected breath by breath. Every sigh method uses the canonical belt excursion selected by `config.resp.amp_method`. The default `rolling_median_2x` method is a Genecand-style rolling-median criterion applied to that selected belt excursion; it is not an exact replication of tidal-volume measurements. It flags amplitudes at least twice a centered 15-position rolling median. Only complete centered windows define baselines; the first and last complete baselines are extended unchanged to the recording edges, and each full window requires at least three valid amplitudes. The previous `global_ratio_outlier` and `legacy_60s` methods remain selectable. Set `config.sigh.compare_methods = true` to report and plot rolling/global agreement. Sighs are discrete breath events.
...
- unique detector-specific scientific evidence: `results.detector_diagnostics`, including independently stored asynchrony methods and automatic desaturation event descriptors
...
The HDF5 export schema is `magma_ml_hdf5_v9`. Automatic annotations are stored under `/labels/automatic_mask`, `/events/automatic`, `/burden/automatic`, and `/overlap/automatic`; reviewed annotations retain their corresponding `/reviewed` paths, with coverage under `/labels/review_coverage_mask`. Candidate intervals are under `/events/candidate`, while `/review/provenance`, `/review/history`, and `/review/scope` preserve review metadata. Reviewed breath cycles, derived respiratory evidence, and unique detector evidence have one authoritative copy each under `/resp_cycles`, `/resp_features`, and `/detector_diagnostics`. Asynchrony methods are stored independently below `/detector_diagnostics/async/methods`; branch availability and event-specific desaturation descriptors are below `/detector_diagnostics/desat`. Periodic-breathing evidence remains below `/detector_diagnostics/periodic`. References remain under `/session_reference`, `/resp_reference`, and `/spo2_reference`. Evidence summaries are under `/evidence_summary`, and the resolved recording configuration is under `/config`.
