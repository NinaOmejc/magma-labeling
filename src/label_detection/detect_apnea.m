...
% diagnostics separates per-belt amplitude use, raw fallback use, and unavailable
% endpoints. boundary_info retains candidate/localized support and uncertainty.
    ...
    [events, boundary_records] = localize_apnea_candidates( ...
        candidate_events, amplitude_local_mask, amplitude_state_mask, ...
    ...
    boundary_info = make_label_boundary_info('apnea', 'detect_apnea', ...
        'preferred_breath_or_raw_excursion_fallback_localization', ...
        candidate_events, events, NaN, ...
        'breath_amplitude_or_raw_excursion_fallback', ...
        diagnostics.combined_endpoint_mask, candidate_state_mask, ...
...
% boundaries carry window_sec uncertainty. records preserve evidence provenance.
    ...
    for i = 1:numel(candidates)
        ...
        if has_raw && any(raw_window_local & in_candidate)
            ...
            method = 'raw_excursion_fallback_window_support_localization';
            if has_amp
                source = 'mixed';
            else
                source = 'raw_excursion_fallback';
            end
