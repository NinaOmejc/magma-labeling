...
YY=freq; XX=(0:(L-1))/fs; ZZ=abs(WT);  ZZ=ZZ.^2;
...
if ~strcmpi(PlotMode,'off')
    ...
    if ~isempty(strfind(lower(PlotMode),'pow+tm'))
        
        close gcf;


        scrsz=get(0,'ScreenSize'); figure('Position',[scrsz(3)/4,scrsz(4)/8,scrsz(3)/1.5,6*scrsz(4)/8]);
       ...
       axes('Position',[0.1,0.15,0.6,0.53],'Layer','top','YScale','log','Box','on','FontSize',16);
        ...
        axis tight
                        ax = axis;      
                        sirnina_crt=0.5;
                        frekvence = [0.6 4 7.5 14 22 40];
                        l = line([ax(1) ax(2)],[frekvence(1) frekvence(1)]);
                        set(l,'color','k','linewidth',sirnina_crt,'linestyle','--'); 
                        l = line([ax(1) ax(2)],[frekvence(2) frekvence(2)]);
                        set(l,'color','k','linewidth',sirnina_crt,'linestyle','--'); 
                        l = line([ax(1) ax(2)],[frekvence(3) frekvence(3)]);
                        set(l,'color','k','linewidth',sirnina_crt,'linestyle','--'); 
                        l = line([ax(1) ax(2)],[frekvence(4) frekvence(4)]);
                        set(l,'color','k','linewidth',sirnina_crt,'linestyle','--'); 
                        l = line([ax(1) ax(2)],[frekvence(5) frekvence(5)]);
                        set(l,'color','k','linewidth',sirnina_crt,'linestyle','--'); 
                        l = line([ax(1) ax(2)],[frekvence(6) frekvence(6)]);
                        set(l,'color','k','linewidth',sirnina_crt,'linestyle','--'); 
        ...
        coib1(coib1==0)=NaN; coib2(coib2==0)=NaN;
        ib=find(L-coib1-coib2>0,1,'first'); if isempty(ib), ib=1; end
        plot(coib1(end:-1:ib)/fs,freq(end:-1:ib),'-k','LineWidth',2);
        plot((L-coib2(ib:end)+1)/fs,freq(ib:end),'-k','LineWidth',2);
        plot([coib1(ib)/fs,(L-coib2(ib)+1)/fs],[freq(ib),freq(ib)],'-k','LineWidth',2);
        coib1(isnan(coib1))=0; coib2(isnan(coib2))=0; if strcmpi(CutEdges,'off'), coib1(:)=0; coib2(:)=0; end
