...
% moving-window energies, and a MAGMA-adapted 0.60 threshold. MAGMA combines
% belts dynamically and uses twice the configured energy-window duration as
% the sustained event requirement.
    ...
    diagnostics = struct( ...
        ...
        'implementation', ['MAGMA adaptation of the published method with ' ...
            'configured threshold and persistence settings'], ...
...
% VALIDATE_EAMI_CONFIG Validate filter, rate, energy, and threshold settings.
    ...
    if ~isstruct(cfg) || ~all(isfield(cfg, required))
        error('MAGMA:Periodic:InvalidEAMIConfig', ...
            'config.periodic.eami is incomplete.');
    end
    ...
    for i = 1:numel(positive_scalars)
        ...
        if ~isscalar(value) || ~isfinite(value) || value <= 0
            error('MAGMA:Periodic:InvalidEAMIConfig', ...
                'config.periodic.eami.%s must be positive.', positive_scalars{i});
        end
    end
