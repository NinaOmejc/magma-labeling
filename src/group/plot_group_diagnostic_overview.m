...
% Each element maps authoritative saved value/time paths to the corresponding
% trace_* group-summary prefix, display labels, and output filename stem.
    ...
    specs(end+1) = make_signal_spec('resp_features', ...
        'diaph.rate_slow_window_bpm', 'resp_features', 'time_sec', ...
    ...
    specs(end+1) = make_signal_spec('detector_diagnostics', ...
        'async.phase_coherence_mid', 'resp_features', 'time_sec', ...
    ...
    specs(end+1) = make_signal_spec('resp_features', ...
        ...
        'resp_features', 'time_sec', 'trace_thoracic_to_abdominal_ratio', ...
    ...
    specs(end+1) = make_signal_spec('resp_features', ...
        ...
        'resp_features', 'time_sec', 'trace_thoracic_dominance_log_ratio', ...
    ...
    specs(end+1) = make_signal_spec('resp_features', ...
        ...
        'resp_features', 'time_sec', 'trace_thoracic_relative_fraction', ...
...
function spec = make_signal_spec(source, field_path, time_source, time_path, ...
        summary_prefix, title_text, ylabel_text, file_stub)
% MAKE_SIGNAL_SPEC Create signal spec.
% Returns a scalar struct mapping saved value/time paths to its summary column
% prefix, display title/ylabel, and output filename stem.
...
function save_group_figure(~, file_path, config)
