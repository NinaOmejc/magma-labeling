...
%   length numel(peak_t)-1.
    ...
    phys_feat.resp.temporal_semantics = struct( ...
        ...
        'inferred_state', 'union of the complete support intervals of qualifying windows', ...
        'minimum_event_duration', 'minimum duration applied once to the inferred state');
...
function balance = build_thoracoabdominal_balance(lungs, diaph, t_grid, cfg)
    ...
    % Compatibility alias: dominance_mask now denotes inferred state, not
    % delayed endpoint evidence.
