...
%   length numel(peak_t)-1.
    ...
    cycle_provenance = respiratory_cycle_provenance(resp_cycles);
    resp_features = struct();
    resp_features.version = 'independent_respiratory_evidence_v5';
    resp_features.provenance = struct( ...
        'cycle_source', 'resp_cycles', ...
        'cycle_review_status', cycle_provenance.review_status, ...
        'manual_review_performed', cycle_provenance.manual_review_performed, ...
        'manual_edits_made', cycle_provenance.manual_edits_made, ...
        'loaded_from_cache', cycle_provenance.loaded_from_cache, ...
        ...
        'version_changes', ['v5: renamed respiratory evidence and removed SpO2; ' ...
            'v4: Slow and Rapid use 60-second confirmation windows ' ...
    ...
    resp_features.resp.alignment_convention = [ ...
    ...
    resp_features.resp.temporal_semantics = struct( ...
        ...
        'localized_state', ['detector-specific boundary refinement from respiratory cycles or raw ' ...
    ...
    resp_features.resp.lungs = build_belt_evidence( ...
        get_belt(resp_cycles, 'lungs'), get_belt(resp_ref, 'lungs'), ...
        lungs_ignored, t_grid, cfg, config);
    resp_features.resp.diaph = build_belt_evidence( ...
        get_belt(resp_cycles, 'diaph'), get_belt(resp_ref, 'diaph'), ...
        false, t_grid, cfg, config);
    resp_features.resp.belt_availability = struct( ...
        'lungs', resp_features.resp.lungs.available, ...
        'diaph', resp_features.resp.diaph.available);
    resp_features.resp.both_belts_available = ...
        resp_features.resp.lungs.available && resp_features.resp.diaph.available;
    resp_features.resp.thoracoabdominal_balance = build_thoracoabdominal_balance( ...
        resp_features.resp.lungs, resp_features.resp.diaph, t_grid, cfg);
end
...
function provenance = respiratory_cycle_provenance(resp_cycles)
    ...
    if isstruct(resp_cycles) && isfield(resp_cycles, 'provenance') && ...
            isstruct(resp_cycles.provenance)
        source = resp_cycles.provenance;
        if isfield(source, 'review_status')
            candidate = char(string(source.review_status));
            if ismember(candidate, {'automatic', 'manual_reviewed_unchanged', ...
                    'manual_reviewed_edited'})
                review_status = candidate;
            end
        end
        if isfield(source, 'loaded_from_cache') && isscalar(source.loaded_from_cache)
            loaded_from_cache = logical(source.loaded_from_cache);
        end
    end
    provenance = struct( ...
        'review_status', review_status, ...
        'manual_review_performed', ~strcmp(review_status, 'automatic'), ...
        'manual_edits_made', strcmp(review_status, 'manual_reviewed_edited'), ...
...
function belt = build_belt_evidence(source, reference, ignored, t_grid, cfg, config)
    ...
    % Direct provenance fields: preserve the respiratory-cycle arrays and
    % their trailing-NaN convention exactly as supplied by resp_cycles.
