...
% Freeze automatic weak annotations and construct a separate reviewed layer.
...
% gold_review_mask is TRUE. Unreviewed samples remain unknown.
    ...
    for i = 1:numel(defs)
        ...
        if ~any(strcmp(reviewed_fields, defs(i).field))
            continue;
        end
        reviewed_parts{end+1} = field_events(reviewed_event_sets, defs(i).field); %#ok<AGROW>
    ...
    if isstruct(sigh_review) && isfield(sigh_review, 'reviewed') && sigh_review.reviewed
    ...
    annotations = struct( ...
        'version', 'weak_reviewed_annotations_v1', ...
