...
% are zeroed before analysis.
    ...
    selected_labels = resolve_selected_labels(config);
    ...
    do_analysis = should_analyze_recording( ...
        result_exists, config.overwrite_results, mode, selected_labels);
    if result_exists
        if ~isempty(selected_labels)
            log_message(config, 1, ...
                ['Existing result found; selected labels will be updated: ' ...
                 'Sub %d | Measurement: %d'], ...
                config.subject, config.measure);
        elseif strcmp(mode, 'review_only')
            log_message(config, 1, ...
