function summary = build_label_evidence_summary( ...
    ...
    detector_diagnostics, label_burden, config, annotation_mask, ...
    annotation_events, layer_assessable_mask)
...
% layer_assessable_mask is the physiological scope for automatic summaries
% and the physiological-plus-explicit-review scope for reviewed summaries.
...
% specific counts; unavailable statistics remain NaN.
    ...
    if nargin < 12 || isempty(layer_assessable_mask)
        layer_assessable_mask = repmat( ...
            logical(label_available(:)'), size(data, 1), 1);
    end
    if ~isequal(size(layer_assessable_mask), size(annotation_mask))
        error('MAGMA:Evidence:AssessableMaskSize', ...
            'layer_assessable_mask must align with annotation_mask.');
    end
    layer_assessable_mask = logical(layer_assessable_mask);
    summary = struct('version', 'detector_specific_evidence_summary_v6', ...
    ...
    deep_scope = label_column(layer_assessable_mask, label_names, 'deep');
    rapid_scope = label_column(layer_assessable_mask, label_names, 'rapid');
    irregular_scope = label_column( ...
        layer_assessable_mask, label_names, 'irregular');
    thoracic_scope = label_column( ...
        layer_assessable_mask, label_names, 'thoracic');
    async_scope = label_column(layer_assessable_mask, label_names, 'async');
    ...
    summary.deep.recording_median_ratio_lungs = finite_median( ...
        breath_values_in_label( ...
            lungs.amp_ratio_session, lungs.peak_t, deep_scope, config.fs));
    summary.deep.recording_median_ratio_diaph = finite_median( ...
        breath_values_in_label( ...
            diaph.amp_ratio_session, diaph.peak_t, deep_scope, config.fs));
    ...
    summary.rapid.recording_median_rr_lungs = finite_median( ...
        grid_values_in_label(lungs.rate_rapid_window_bpm, ...
            resp_features.time_sec, rapid_scope, config.fs));
    summary.rapid.recording_median_rr_diaph = finite_median( ...
        grid_values_in_label(diaph.rate_rapid_window_bpm, ...
            resp_features.time_sec, rapid_scope, config.fs));
    ...
    summary.irregular.recording_median_cov_lungs = finite_median( ...
        grid_values_in_label(lungs.irregularity.cov, ...
            resp_features.time_sec, irregular_scope, config.fs));
    summary.irregular.recording_median_cov_diaph = finite_median( ...
        grid_values_in_label(diaph.irregularity.cov, ...
            resp_features.time_sec, irregular_scope, config.fs));
    ...
    summary.thoracic.recording_median_thoracic_to_abdominal_ratio = ...
        finite_median(grid_values_in_label( ...
            balance.thoracic_to_abdominal_ratio, resp_features.time_sec, ...
            thoracic_scope, config.fs));
    if logical(optional_field(balance, 'available', false))
        summary.thoracic.supporting_belts = 'both';
    else
        summary.thoracic.supporting_belts = '';
    end
    ...
    summary.async.recording_median_absolute_phase_deg = NaN;
    if isfield(rea, 'methods') && ...
        ...
        phase_reliable = optional_field(phase, ...
            'reliable_phase_mask', true(size(phase.absolute_mean_phase_deg)));
        recording_absolute_phase = grid_values_in_scope_and_evaluable( ...
            phase.absolute_mean_phase_deg, phase_reliable, phase_time, ...
            async_scope, config.fs);
        summary.async.recording_median_absolute_phase_deg = ...
            finite_median(recording_absolute_phase);
...
function values = grid_values_in_scope_and_evaluable( ...
    values, evaluable_mask, time_sec, sample_scope, fs)
% GRID_VALUES_IN_SCOPE_AND_EVALUABLE Select reliable layer-specific evidence.

    values = values(:);
    evaluable_mask = logical(evaluable_mask(:));
    time_sec = time_sec(:);
    if numel(values) ~= numel(evaluable_mask) || ...
            numel(values) ~= numel(time_sec)
        values = zeros(0, 1);
        return;
    end
    in_scope = sample_mask_at_times(sample_scope, time_sec, fs);
    values = values(in_scope & evaluable_mask & isfinite(values));
end
