...
% SAVE_FIGURE Standardize, export, and close the current pipeline figure.
%
% Inputs:
%   config      - Figure size, visibility, format, resolution, and result path.
%   base_name   - Filename stem appended to subject and measurement identifiers.
%   save_matfig - Whether to write an editable .fig sidecar.
%   output_path - Optional complete destination path overriding generated naming.
...
% SAVE_FIG_FOR_DIRECT_OPEN Save an editable figure that reopens visibly.
...
% STRENGTHEN_DASHED_LINES Enforce a minimum width for dashed figure objects.
% target_width is in points and applies to "--" and "-." line styles.
...
% RESOLVE_TARGET_VISIBILITY Return configured MATLAB figure visibility.
...
% RESOLVE_PLOT_FORMAT Validate the requested extension, defaulting to PNG.
...
% RESOLVE_PLOT_DPI Return a positive export resolution in dots per inch.
